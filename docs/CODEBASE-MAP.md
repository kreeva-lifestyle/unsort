...
| `minis/ratecard/renderPages.ts` | 44 | **Pages** output (owner's sample): two SKUs per page on a white sheet, each photo whole (contain, bottom-aligned) with its code right-aligned beneath; no backdrop, frame or logo; lone last photo centred | pure |
