...
| PWA | `public/sw.js` precaches `/index.html`, network-first for HTML/JS/CSS, cache-first for public Supabase Storage images (own `dailyoffice-images-v1` cache that survives deploys, CORS refetch on miss, capped at 200 oldest-first; uploads set `cacheControl: '31536000'` since every upload is a new url), posts `SW_UPDATED` to clients on activate; `main.tsx` shows a full-screen "Update Available" overlay only when a Supabase auth token exists in `localStorage` and the hash is not a `#/s/` short link. `public/manifest.json`, icons, `arya-designs-logo.png` (preloaded for short-link pages). |
...
| `components/challan/ChallanForm.tsx` | 451 | Create/edit/return form (parent owns save); payment boxes live in `PaymentFields` | RPC `get_next_challan_number`; `cash_challans` recent, `cash_challan_customers` phone |
| `components/contacts/Contacts.tsx` | 70 | Shared Contacts sub-view (Cash Challan + Purchase Orders): one list of every customer AND supplier, search (name/phone/address), All/Customers/Suppliers filter, "+ New contact"; `useCrumb('Contacts')`; parent owns the Back entry | `cash_challan_customers`, `po_vendors` via contactsModel |
| `components/contacts/contactsMerge.ts` | 73 | Pure: `mergeContacts` (a customer and a supplier are one contact when phones match or names match ignoring case/space), `filterContacts`, `formOf`, `validateForm` | — |
| `components/contacts/contactsModel.ts` | 26 | `loadContacts` (both masters, explicit columns, 2000 cap) + `saveContact` → RPC `save_contact` | RPC `save_contact` |
| `components/contacts/ContactList.tsx` | 54 | Rows with avatar letter, Customer/Supplier/Inactive chips, phone · address · GSTIN; `<Empty>` | — |
| `components/contacts/ContactModal.tsx` | 118 | Add/edit: name, phone, role chips (an existing role is locked — documents reference it), address; supplier-only GSTIN / active toggle / notes. Issued docs keep their snapshot | RPC `save_contact` |
| `components/challan/PaymentFields.tsx` | 77 | Status / Payment Mode / Amount Paid / Payment Date grid; owns `PAYMENT_MODES`. Shortcut: Status → Paid fills the total (never lowers a larger typed amount) and dates today; the FULL chip inside Amount Paid does the same from any status | — |
