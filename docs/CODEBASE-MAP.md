...
| `minis/ratecard/IndexMaker.tsx`, `IndexTile.tsx`, `renderIndex.ts`, `indexBackdrop.ts`, `indexPhotos.ts`, `useScriptFont.ts` | 158, 49, 118, 66, 63, 25 | Index maker: photos + SKU each (SKU pre-filled from file name, reorder, sort, duplicate warning; 320 px thumbnails made 3 at a time, memoised cards) → at Generate the photos decode straight to tile size 4 at a time with progress, then a canvas JPEG: grid captioned with SKUs + gold ornament, logo beside (landscape, 4 cols) or above (portrait, 3 cols), backdrop = blurred mosaic of the photos under a scrim tinted with their average colour (Dark / Light mood), shadow stamped from one sprite, optional script title | `/arya-designs-logo.png`; Google font fetch; result via `RateCardActions` (`fileLabel="Index"`) |
...
**pg_cron jobs (9):** `master-sync-probe` `*/2 * * * *`, `master-sync-full` `23 * * * *`, `master-spellfix-weekly` `30 2 * * 0`, `product-catalog-refresh` `*/2 * * * *`, `purge_listing_runs` `17 3 * * *`, `purge-otp-inbox` `50 3 * * *` (30 d), `purge-cron-history` `40 3 * * *` (7 d), `costing-top-subs` `10 3 */4 * *`, `po-top-vendors` `20 3 * * *`. `cleanup_old_link_clicks()` exists but is **not** scheduled.
