...
| `minis/ratecard/renderPages.ts` | 41 | **Pages** output: two photos edge to edge at one height (1600 px, whole, never cropped; page width = the two widths), each code INSIDE its photo bottom-right in white with a soft shadow; no margin, gap, backdrop or frame; an odd last page keeps the two-photo width and fills the empty half with a brand panel (logo + catalog name over a blurred, tinted copy of that photo). Decoded per page at 1600 px, released after drawing | `/arya-designs-logo.png` (odd count only) |
