...
| `minis/ratecard/renderPages.ts` | 41 | **Pages** output: two photos edge to edge at one height (1600 px, whole, never cropped; page width = the two widths), each code INSIDE its photo bottom-right in white with a soft shadow; no margin, gap, backdrop, frame or logo; lone last photo is a page alone. Decoded per page at 1600 px, released after drawing | pure |
