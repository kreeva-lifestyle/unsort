...
function App(){
  ...
  const handlePaletteAction = (item)=>{
    ...
    if(item.kind==='nav'){
      const map = {Dashboard:'dashboard',Inventory:'inventory','Brand Tags':'brandtags',
        PackStation:'packstation','Cash Challan':'challan',CashBook:'cashbook',Settings:'dashboard'};
  ...
  const renderScene = ()=>{
    ...
    return <div style={{padding:'40px 32px',maxWidth:1200,margin:'0 auto',color:T.tx2}}>
      <div style={{fontFamily:T.sora,fontSize:24,color:T.tx,marginBottom:8}}>
        {({brandtags:'Brand Tags',packstation:'PackStation',challan:'Cash Challan',cashbook:'CashBook'})[scene]}
      </div>
      <div>This screen exists in the real app — focus here is the three UX upgrades. Use ⌘K to jump back, or try Inventory for undo+history, or Dashboard for the cockpit.</div>
    </div>;
  };
