-- Raise POs from a product costing sheet (owner's ask, phase 1 of wiring
-- PO ↔ costing). The purchase plan already says "to make N pcs of SKU, buy
-- this from these suppliers"; the app now turns each supplier block into
-- a draft PO. The PO remembers which costing it came from:
--   purchase_orders.costing_product_id → costing_products (set null if the
--   costing is deleted, the PO stands on its own). Explicit link only —
--   never guessed from names.
-- create_po_with_items / update_po_with_items accept it in p_po; the rest
-- of both bodies is unchanged from 20260916203000.

alter table public.purchase_orders
  add column if not exists costing_product_id uuid references public.costing_products(id) on delete set null;
create index if not exists idx_purchase_orders_costing_product on public.purchase_orders (costing_product_id) where costing_product_id is not null;
