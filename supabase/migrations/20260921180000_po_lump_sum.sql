-- "Lump sum" on a purchase order (owner's ask): some orders are priced as a
-- whole, not per piece, so "For how many pcs" makes no sense for them. A
-- tick on the form marks the order lump sum and the piece count is then
-- optional (stored NULL). Everything else keeps the rule from
-- 20260920140000: not lump sum → pieces required, a whole number above 0.
-- Both RPC bodies are the 20260920140000 ones with only the lump-sum
-- handling added (v_lump, the pieces check, the column in insert/update
-- and the audit diff).
alter table public.purchase_orders add column if not exists lump_sum boolean not null default false;
comment on column public.purchase_orders.lump_sum is 'Priced as a whole: for_pieces is optional (NULL) on such orders.';
