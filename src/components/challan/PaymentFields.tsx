      ...
      <div>
        ...
        <div style={{ position: 'relative' }}>
          <input type="number" min="0" value={amount || ''} onKeyDown={e => numericKeyDown(e)} onChange={e => setAmount(Math.max(0, Number(e.target.value)))}
            placeholder="Amount received" style={{ ...inp, fontFamily: T.mono, fontSize: 11, paddingRight: canFill ? 60 : undefined }} />
          ...
          {canFill && (
            <button type="button" onClick={fillFull} title={`Full amount — ₹${total.toLocaleString('en-IN')}`} aria-label={`Fill the full amount, ₹${total.toLocaleString('en-IN')}`}
              style={{ position: 'absolute', right: 4, top: 4, bottom: 4, padding: '0 9px', borderRadius: 6, border: `1px solid ${T.ac33}`, background: T.ac3, color: T.ac2, fontSize: 10, fontWeight: 700, letterSpacing: '.06em', cursor: 'pointer', fontFamily: T.sans }}>
              FULL
            </button>
          )}
        </div>
