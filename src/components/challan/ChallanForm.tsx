          ...
          <div className="challan-form-grid-2" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10, marginBottom: outstanding > 0 ? 4 : 12 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
                <span style={{ padding: '8px 10px', background: T.glass1, border: `1px solid ${T.bd}`, borderRight: 'none', borderRadius: '8px 0 0 8px', fontSize: 12, color: T.tx3, fontFamily: T.mono, whiteSpace: 'nowrap', lineHeight: '1.2' }}>+91</span>
            ...
            {p.items.map((it, i) => {
              ...
              <div key={i}>
                <div className="challan-item-row" style={{ display: 'grid', gridTemplateColumns: '1fr 92px 80px 100px 28px', gap: 6, padding: '7px 12px', borderBottom: err ? 'none' : `1px solid ${T.bd}`, alignItems: 'center' }}>
                  ...
                  <button type="button" className="challan-remove" onClick={() => { if (p.items.length > 1) p.setItems(p.items.filter((_, j) => j !== i)); }} style={{ border: 'none', background: 'none', color: T.re, cursor: 'pointer', fontSize: 18, padding: '4px 8px', minWidth: 36, minHeight: 36, opacity: 0.7 }} aria-label="Remove item">&#215;</button>
