...
export default function QuickChips({ navigateTo }: { navigateTo?: (tab: string) => void }) {
  ...
  const chip = (s: Shortcut, i: number) => (
    <div key={s.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
      {editing && <button type="button" onClick={() => move(i, -1)} disabled={i === 0} aria-label={`Move ${s.label} earlier`} style={{ ...S.btnGhost, ...S.btnSm, minHeight: 30, padding: '4px 6px', opacity: i === 0 ? 0.3 : 1 }}>‹</button>}
      <button type="button" onClick={() => (editing ? save(chips.filter(c => c.id !== s.id)) : openShortcut(s, navigateTo))} aria-label={editing ? `Remove ${s.label}` : `Open ${s.label}`}
      ...
      {editing && <button type="button" onClick={() => move(i, 1)} disabled={i === chips.length - 1} aria-label={`Move ${s.label} later`} style={{ ...S.btnGhost, ...S.btnSm, minHeight: 30, padding: '4px 6px', opacity: i === chips.length - 1 ? 0.3 : 1 }}>›</button>}
    </div>
  );
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        {custom && <button type="button" onClick={() => setEditing(e => !e)} style={{ border: 'none', background: 'none', color: editing ? T.ac2 : T.tx3, fontSize: 11, cursor: 'pointer', padding: '4px 6px', minHeight: 30 }}>{editing ? 'Done' : 'Edit'}</button>}
      </div>
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: 2, flexWrap: 'wrap' }}>
        {chips.map(chip)}
        {!editing && chips.length < MAX_QUICK_CHIPS && (
          <button type="button" onClick={() => setPicking(true)} aria-label="Add a shortcut" style={{ ...S.btnGhost, ...S.btnSm, minHeight: 34, padding: '5px 12px', fontSize: 11, borderRadius: 999, borderStyle: 'dashed', flexShrink: 0 }}>+ Add</button>
        )}
      </div>
