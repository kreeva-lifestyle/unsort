...
export default function TemplateManager({ open, onClose, templates, refresh, addToast }: {
  ...
  useBackClose(showRules, () => setShowRules(false)); // rules view → back to the template, not out of the manager
    ...
    <div style={S.modalOverlay} onClick={editing ? requestLeave : onClose}>
      <div className="modal-inner" style={{ ...S.modalBox, width: 560 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: 16, overflowY: 'auto' }}>
          <input ref={fileRef} type="file" accept=".xlsx,.xls,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/csv" onChange={onFile} style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden', opacity: 0 }} />
          {!editing && <>
