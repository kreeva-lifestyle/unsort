...
export default function Toggle({ on, onToggle, size = 'md' }: { on: boolean; onToggle: () => void; size?: 'sm' | 'md' }) {
  ...
  const pad = (h - dot) / 2;
  return (
    <div onClick={onToggle} style={{ width: w, height: h, borderRadius: h, background: on ? T.gr : 'rgba(255,255,255,.12)', cursor: 'pointer', position: 'relative', transition: 'background .2s ease', flexShrink: 0, boxShadow: on ? `0 0 12px oklch(0.72 0.19 145 / .25)` : 'inset 0 1px 3px rgba(0,0,0,.3)' }}>
