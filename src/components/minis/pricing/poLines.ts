...
import { itemLabel } from '../../purchaseorders/poItemLabel';
...
interface Row { id: string; item_name: string; fabric_code: string | null; sku: string | null; quantity: number | string; unit: string | null; rate: number | string | null; purchase_orders: { po_number: number; vendor_name: string; status: string; po_date: string | null } | { po_number: number; vendor_name: string; status: string; po_date: string | null }[] | null }
...
export async function loadPoLines(): Promise<{ lines: PoLine[]; error: unknown }> {
  ...
  const { data, error } = await supabase.from('purchase_order_items')
    .select('id, item_name, fabric_code, sku, quantity, unit, rate, purchase_orders!inner(po_number, vendor_name, status, po_date)')
  ...
  const lines: PoLine[] = ((data ?? []) as unknown as Row[]).flatMap(r => {
    ...
    return [{ id: r.id, po: po.po_number, vendor: (po.vendor_name || '').trim(), status: po.status, date: po.po_date, sku: r.sku, item: itemLabel({ item_name: (r.item_name || '').trim(), fabric_code: r.fabric_code }), qty: Number(r.quantity) || 0, unit: r.unit, rate: rate !== null && Number.isFinite(rate) && rate > 0 ? rate : null }];
  });
