...
export default function ForwardDropbox({ addToast, onBack }: { addToast: (m: string, t?: string) => void; onBack: () => void }) {
  ...
  const startCam = useCallback(() => {
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment', width: { ideal: 1920 }, height: { ideal: 1080 } } })
      .then(stream => { if (!videoRef.current) { stream.getTracks().forEach(t => t.stop()); return; } streamRef.current = stream; v.srcObject = stream; v.onloadedmetadata = () => { v.play().catch(() => setCameraError('Cannot start the camera. Check the camera permission for this site.')); }; })
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 140, background: '#05070c', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 6, padding: 'max(10px, env(safe-area-inset-top)) 12px 14px', display: 'flex', gap: 8, alignItems: 'center', background: 'linear-gradient(180deg, rgba(0,0,0,.6), transparent)' }}>
        <div onClick={() => { stopCam(); onBack(); }} style={ic} aria-label="Back">‹</div>
        <div style={{ flex: 1, maxWidth: 240, display: 'flex', alignItems: 'center', background: 'rgba(10,13,20,.62)', border: '1px solid rgba(255,255,255,.16)', borderRadius: 12, overflow: 'hidden', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }}>
          <div onClick={() => setDateStr(addDays(dateStr, -1))} style={{ width: 36, height: 38, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 17, cursor: 'pointer' }}>‹</div>
          <div onClick={openDate} style={{ flex: 1, textAlign: 'center', padding: '4px 2px', cursor: 'pointer' }}>
        ...
        <input ref={dateInputRef} type="date" value={dateStr} onChange={e => e.target.value && setDateStr(e.target.value)} style={{ position: 'absolute', width: 1, height: 1, opacity: 0, pointerEvents: 'none' }} />
      </div>
      ...
      {mode === 'camera' ? (
        <>
          <div ref={camRef} style={{ flex: 1, background: 'linear-gradient(160deg,#10151f,#0a0d15 70%)', overflow: 'hidden' }} />
          {/* corner guides */}
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 6, padding: '14px 14px calc(16px + env(safe-area-inset-bottom))', display: 'flex', flexDirection: 'column', gap: 10, background: 'linear-gradient(0deg, rgba(5,7,12,.97), rgba(5,7,12,.75) 70%, transparent)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,.05)', border: `1px solid ${T.bd2}`, borderRadius: 9, overflow: 'hidden' }}>
                <div onClick={() => setDateStr(addDays(dateStr, -1))} style={{ width: 30, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', color: T.ac2, fontSize: 15, cursor: 'pointer' }}>‹</div>
