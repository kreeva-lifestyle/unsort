...
interface Label { id: string; label_text: string; created_at: string }

const esc = (s: unknown) => String(s ?? '').replace(/[<>"'&]/g, c => ({ '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;', '&': '&amp;' }[c] || c));

const buildPrintHtml = (items: { text: string; copies: number }[]): string => {
  const today = new Date().toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' });
  const labels = items.flatMap(it =>
    Array.from({ length: it.copies }, () =>
      `<div class="label">
  <div class="inner">
    <div class="banner"><span class="warn">&#9888;</span> RETURN</div>
    <div class="reason">${esc(it.text.toUpperCase())}</div>
    <div class="footer">
...
.warn{font-size:14pt;margin-right:4px}
...
export default function ReturnLabels({ addToast }: { addToast: (msg: string, type?: string) => void }) {
  ...
  const fetch = useCallback(async () => {
    const { data, error } = await supabase.from('return_labels').select('id, label_text, created_at').order('created_at', { ascending: false });
  ...
  const filtered = useMemo(() => {
    ...
    return labels.filter(l => l.label_text.toLowerCase().includes(q));
  }, [labels, search]);
  ...
  const openAdd = () => { setEditId(null); setText(''); setFormError(''); setShowModal(true); };
  const openEdit = (l: Label) => { setEditId(l.id); setText(l.label_text); setFormError(''); setShowModal(true); };
  const closeModal = () => { setShowModal(false); setEditId(null); setText(''); setFormError(''); };
  ...
  const save = async () => {
    ...
    try {
      if (editId) {
        const { error } = await supabase.from('return_labels').update({ label_text: trimmed }).eq('id', editId);
        if (error) throw error;
        addToast('Label updated', 'success');
      } else {
        const { error } = await supabase.from('return_labels').insert({ label_text: trimmed });
        if (error) throw error;
  ...
  const doDelete = async (id: string) => {
    ...
    if (error) addToast(friendlyError(error), 'error');
    else { addToast('Label deleted', 'success'); fetch(); setPage(0); }
  ...
  const printAll = () => {
    const items = filtered.filter(l => getCopies(l.id) > 0).map(l => ({ text: l.label_text, copies: getCopies(l.id) }));
    if (items.length === 0) { addToast('Nothing to print', 'error'); return; }
    setPrintHtml(buildPrintHtml(items));
  };

  const printOne = (l: Label) => {
    setPrintHtml(buildPrintHtml([{ text: l.label_text, copies: getCopies(l.id) }]));
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 14, alignItems: 'center' }}>
        ...
        <button onClick={printAll} disabled={filtered.length === 0} style={{ ...S.btnGhost, opacity: filtered.length === 0 ? 0.3 : 1 }}>
      ...
      {filtered.length > 0 ? (
          ...
          <div className="desktop-only" style={S.tableWrap}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>
                ...
                <th style={{ ...S.thStyle, width: 160, textAlign: 'right' }}>Actions</th>
              </tr></thead>
              <tbody>{paged.map(l => (
                <tr key={l.id}>
              ...
              <div key={l.id} style={{ background: T.glass1, border: `1px solid ${T.bd}`, borderRadius: 10, padding: '14px 14px 12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={() => printOne(l)} style={{ ...S.btnGhost, ...S.btnSm, minHeight: 36, padding: '6px 12px' }}>Print</button>
                    <button onClick={() => openEdit(l)} style={{ ...S.btnGhost, ...S.btnSm, minHeight: 36, padding: '6px 12px' }}>Edit</button>
                    <button onClick={() => setConfirmDelete(l.id)} style={{ ...S.btnDanger, ...S.btnSm, minHeight: 36, padding: '6px 12px' }}>Del</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <Empty icon="tag" title={search ? 'No labels match your search' : 'No return labels yet'} message={search ? 'Try a different search term' : 'Add your first return label to get started'} cta={search ? undefined : 'Add Label'} onCta={search ? undefined : openAdd} />
      )}
        ...
        <div style={S.modalOverlay} onClick={closeModal}>
          <div className="modal-inner" style={{ ...S.modalBox, maxWidth: 420 }} onClick={e => e.stopPropagation()}>
            <div style={S.modalHead}>
              <h3 style={S.modalTitle}>{editId ? 'Edit Label' : 'Add Return Label'}</h3>
            ...
            <div style={{ padding: '16px 18px' }}>
              ...
              {formError && <div style={{ ...S.errorBox, marginTop: 10 }}>{formError}</div>}
      ...
      {printHtml && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: '#060810', display: 'flex', flexDirection: 'column', touchAction: 'none' }}>
          <div style={{ padding: '12px 16px', paddingTop: 'max(12px, env(safe-area-inset-top))', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,.08)', background: 'rgba(8,11,20,.95)', backdropFilter: 'blur(20px)' }}>
            <div>
              <span style={{ fontSize: 13, fontWeight: 600, color: '#E2E8F0', fontFamily: "'Sora',sans-serif" }}>Return Label Preview</span>
          ...
          <iframe ref={printRef} title="Return label print preview" srcDoc={printHtml} style={{ flex: 1, width: '100%', border: 'none', background: '#fff' }} />
          <div style={{ padding: '10px 16px', paddingBottom: 'max(10px, env(safe-area-inset-bottom))', background: 'rgba(8,11,20,.95)', borderTop: '1px solid rgba(255,255,255,.08)', display: 'flex', gap: 10, justifyContent: 'center' }}>
            ...
            <button onClick={() => { printRef.current?.contentWindow?.print(); }} style={{ padding: '10px 24px', borderRadius: 8, border: 'none', background: `linear-gradient(135deg, ${T.ac}, ${T.ac2})`, color: '#fff', fontSize: 13, cursor: 'pointer', fontWeight: 600, flex: 1, maxWidth: 160, boxShadow: '0 4px 16px rgba(99,102,241,.35)' }}>Print</button>
          </div>
        </div>,
        document.body
      )}
