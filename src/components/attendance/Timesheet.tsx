    ...
    <div>
      ...
      <div className="mobile-only" style={{ display: 'none', flexDirection: 'column', gap: 6 }}>
        {rows.map(e => {
          ...
          const emp = empById.get(e.employee_id);
          return (
            <div key={e.id} style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${T.bd}`, borderRadius: 8, padding: '10px 12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: T.tx }}>{emp?.name || '—'}</span>
              ...
              <div style={{ display: 'flex', gap: 12, fontSize: 11, fontFamily: T.mono, color: T.tx2 }}>
                <span>In {e.in_time || '—'}</span><span>Out {e.out_time || '—'}</span>
                <span style={{ color: d ? (d.short ? T.re : T.gr) : T.tx3, fontWeight: 600 }}>{d ? d.txt : '—'}</span>
                <span style={{ marginLeft: 'auto', color: e.status === 'A' ? T.re : T.tx3 }}>{e.status || ''}</span>
              </div>
            </div>
          );
        })}
