...
export default function AttendanceEmployees({ employees, onChanged, addToast }: {
  ...
  const openAdd = () => { setEditing(null); setName(''); setCode(''); setSalary(''); setFixTime('8:30'); setErr(''); setShowModal(true); };
  const openEdit = (e: AttEmployee) => { setEditing(e); setName(e.name); setCode(e.employee_code || ''); setSalary(String(e.salary)); setFixTime(minutesToHM(e.fix_time_minutes)); setErr(''); setShowModal(true); };
  ...
  const save = async () => {
    ...
    setSaving(true);
    const payload = { name: name.trim(), employee_code: code.trim() || null, salary: sal, fix_time_minutes: fixMin, updated_at: new Date().toISOString() };
    const { error } = editing
  ...
  return (
    <div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${T.bd}`, borderRadius: 10, overflow: 'hidden' }}>
        {employees.map(e => (
          <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', borderBottom: `1px solid ${T.bd}`, opacity: e.is_active ? 1 : 0.45 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: T.tx }}>{e.name}{e.employee_code ? <span style={{ fontSize: 10, color: T.tx3, fontFamily: T.mono, marginLeft: 8 }}>{e.employee_code}</span> : null}</div>
