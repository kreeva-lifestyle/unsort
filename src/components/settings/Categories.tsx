...
export default function Categories({ addToast, profile }: { addToast: (msg: string, type?: string) => void; profile: any }) {
  ...
  const compInputRow = (val: string, i: number, total: number, offset = 0) => (
    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
      ...
      {total > 1 && <button type="button" onClick={() => removeCompRow(i)} aria-label="Remove row" style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.re, fontSize: 18, lineHeight: 1, padding: '6px 8px', minWidth: 36, minHeight: 36, flexShrink: 0 }}>&#215;</button>}
    </div>
  );

  return (
    <div>
      {canEdit && <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 10 }}><button type="button" className="touch44" onClick={() => { setSelected(null); setForm({ sku: '', name: '', description: '', category: '' }); setNewComps(['']); setShowModal(true); }} style={S.btnPrimary}>+ Add</button></div>}
      <div className="cat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 10 }}>{categories.map((p) => (<div key={p.id} style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${T.bd}`, borderRadius: T.r, padding: '14px 16px', transition: 'border-color .15s, box-shadow .15s' }} onMouseEnter={e => { e.currentTarget.style.borderColor = T.bd2; e.currentTarget.style.boxShadow = '0 4px 16px rgba(0,0,0,.2)'; }} onMouseLeave={e => { e.currentTarget.style.borderColor = T.bd; e.currentTarget.style.boxShadow = 'none'; }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          ...
          {canEdit && <button type="button" className="touch44" onClick={() => openEdit(p)} style={{ ...S.btnGhost, ...S.btnSm }}>Edit</button>}
        </div>
      ...
      {showModal && createPortal(<div style={S.modalOverlay} onClick={closeModal}><div className="modal-inner" style={{ ...S.modalBox, width: 480 }} onClick={e => e.stopPropagation()}><div style={S.modalHead}><span style={{ fontSize: 13, fontWeight: 600, color: T.tx }}>{selected ? 'Edit' : 'Add'} Category</span><button type="button" onClick={closeModal} style={S.modalClose} aria-label="Close">&#215;</button></div><form onSubmit={handleSubmit} style={{ padding: 16 }}>
        ...
        <div style={{ marginBottom: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><label style={{ ...S.fLabel, margin: 0 }}>Components</label><button type="button" className="touch44" onClick={addCompRow} style={{ ...S.btnPrimary, ...S.btnSm }}>+ Add More</button></div>
        {selected && comps.length > 0 && <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${T.bd}`, borderRadius: T.r, padding: 8, marginBottom: 8 }}>
          {comps.map((c: any, i: number) => (<div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 6, marginBottom: 3, background: 'transparent', border: `1px solid ${T.bd}` }}>
            ...
            {canEdit && <button type="button" className="touch44" onClick={() => deleteComp(c.id)} style={{ ...S.btnDanger, ...S.btnSm }}>Delete</button>}
          </div>))}
        </div>}
        ...
        <div style={{ padding: '12px 0 0', borderTop: `1px solid ${T.bd}`, display: 'flex', justifyContent: 'flex-end', gap: 7 }}><button type="button" onClick={closeModal} style={S.btnGhost}>Cancel</button><button type="submit" disabled={saving} style={{ ...S.btnPrimary, opacity: saving ? 0.5 : 1, pointerEvents: saving ? 'none' : 'auto' }}>{saving ? 'Saving…' : selected ? 'Update' : 'Add Category'}</button></div>
      </form></div></div>, document.body)}

      {showCompModal && selected && createPortal(<div style={S.modalOverlay} onClick={() => setShowCompModal(false)}><div className="modal-inner" style={{ ...S.modalBox, width: 500 }} onClick={e => e.stopPropagation()}><div style={S.modalHead}><div><span style={{ fontSize: 13, fontWeight: 600, color: T.tx }}>Components of "{selected.name}"</span><p style={{ margin: '3px 0 0', fontSize: 10, color: T.tx3 }}>Manage the individual parts of this category</p></div><button type="button" onClick={() => setShowCompModal(false)} style={S.modalClose} aria-label="Close">&#215;</button></div><div style={{ padding: 16 }}>
        {canEdit && <form onSubmit={addCompToExisting} style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${T.bd}`, padding: 12, borderRadius: T.r, marginBottom: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}><p style={{ fontSize: 9, color: T.tx3, textTransform: 'uppercase' as const, letterSpacing: 1, fontWeight: 600, margin: 0 }}>Add Components</p><button type="button" className="touch44" onClick={addCompRow} style={{ ...S.btnGhost, ...S.btnSm }}>+ Add More</button></div>
