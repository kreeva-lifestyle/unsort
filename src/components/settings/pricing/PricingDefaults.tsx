// Profit defaults (the projector prefills these per product) and what the
// maintenance % applies to. Saved as app_settings.pricing_defaults.
import { useState } from 'react';
import { T, S } from '../../../lib/theme';
import { friendlyError } from '../../../lib/friendlyError';
import { numericKeyDown } from '../../../lib/numericInput';
import { PricingDefaults as Defaults, MaintenanceBase, PRICING_KEYS, savePricingKey } from '../../minis/pricing/pricingConfig';
...
export default function PricingDefaults({ defaults, addToast, onSaved }: { defaults: Defaults; addToast: (m: string, t?: string) => void; onSaved: (d: Defaults) => void }) {
  ...
  const [base, setBase] = useState<MaintenanceBase>(defaults.maintenanceBase);
  ...
  const save = async () => {
    ...
    const next: Defaults = { profit: { pct: p, fixed: f }, maintenanceBase: base };
    setSaving(true);
  ...
  return (
    <div style={card}>
      <div style={{ fontSize: 13, fontWeight: 700, color: T.tx }}>Profit and maintenance</div>
      <div style={{ fontSize: 11, color: T.tx3, marginBottom: 10 }}>Target price = (cost + fixed profit) ÷ (1 − profit %). Both can be used together; a product can change them.</div>
      <div className="two-col" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.4fr', gap: 8 }}>
        ...
        <div><label style={S.fLabel}>Maintenance % applies to</label>
          <select value={base} onChange={e => setBase(e.target.value as MaintenanceBase)} style={{ ...S.fInput, cursor: 'pointer' }}>
            <option value="materials">Fabric + material only (as the costing sheet)</option>
            <option value="all">Fabric + material + stitching</option>
          </select></div>
      </div>
