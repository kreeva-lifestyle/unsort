...
export function friendlyError(raw: unknown, fallback = 'Something went wrong. Please try again.'): string {
  ...
  if (l.includes('cash handover period') || l.includes('confirmed and is a permanent') || l.includes('permanent financial record')) return msg; // Pass through specific lock-period errors
