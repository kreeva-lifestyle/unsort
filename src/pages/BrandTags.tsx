    ...
    <div style={{ fontFamily: T.sans, color: T.tx, padding: '14px 16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14, gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button style={{ ...S.btnPrimary, background: T.gr, color: '#000', fontWeight: 700, boxShadow: `0 2px 12px oklch(0.72 0.19 145 / 0.4)`, padding: '8px 16px' }} onClick={() => { const toPrint: BrandTagRow[] = []; rows.forEach(r => { for (let i = 0; i < (r.copies || 0); i++) toPrint.push(r); }); if (toPrint.length > 0) openLabelPrint(toPrint); else addToast('Set copies > 0 on rows to print', 'error'); }}>
