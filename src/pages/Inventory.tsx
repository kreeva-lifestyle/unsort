...
export default function Inventory({ openItemId, onItemOpened, active }: { openItemId?: string | null; onItemOpened?: () => void; active?: boolean }) {
      ...
      <div style={{ background: 'rgba(255,255,255,0.015)', border: `1px solid ${T.bd}`, borderRadius: 8, overflow: 'hidden', display: loading && items.length === 0 ? 'none' : undefined }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 880 }}>
          ...
          <tbody>{paged.map((item) => {
            ...
            <td title={item.serial_number || ''} style={{ ...S.tdStyle, fontFamily: T.mono, color: T.ac2, fontSize: 10 }}>{item.serial_number || '—'}{safeHref(item.link) && <a href={safeHref(item.link)} target="_blank" rel="noopener noreferrer" style={{ marginLeft: 4, color: T.bl, verticalAlign: 'middle' }}><Icon name="link" size={10} /></a>}</td>
  ...
  {form.status === 'unsorted' && <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
    <button type="button" onClick={() => { setMissingComps(new Set()); setDamagedComps(new Set()); }} style={{ padding: '4px 10px', borderRadius: 4, border: '1px solid oklch(0.72 0.19 145 / .2)', background: 'oklch(0.72 0.19 145 / .06)', color: T.gr, fontSize: 10, fontWeight: 600, cursor: 'pointer', minHeight: 32 }}>All Present</button>
    <button type="button" onClick={() => { setMissingComps(new Set(catComps.map((c: any) => c.id))); setDamagedComps(new Set()); }} style={{ padding: '4px 10px', borderRadius: 4, border: '1px solid oklch(0.78 0.18 75 / .2)', background: 'oklch(0.78 0.18 75 / .06)', color: T.yl, fontSize: 10, fontWeight: 600, cursor: 'pointer', minHeight: 32 }}>All Missing</button>
  </div>}
  {form.status === 'damaged' && <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
    <button type="button" onClick={() => { setDamagedComps(new Set(catComps.map((c: any) => c.id))); setMissingComps(new Set()); }} style={{ ...S.btnDanger, fontSize: 10, padding: '4px 10px', cursor: 'pointer', minHeight: 32 }}>Mark All Damaged</button>
    <button type="button" onClick={() => { setDamagedComps(new Set()); setMissingComps(new Set()); }} style={{ ...S.btnGhost, fontSize: 10, padding: '4px 10px', minHeight: 32 }}>Reset All</button>
  </div>}
  {form.status === 'dry_clean' && <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
    <button type="button" onClick={() => { setMissingComps(new Set(catComps.map((c: any) => c.id))); setDamagedComps(new Set()); }} style={{ padding: '4px 10px', borderRadius: 4, border: '1px solid rgba(125,211,252,.2)', background: 'rgba(125,211,252,.06)', color: T.bl, fontSize: 10, fontWeight: 600, cursor: 'pointer', minHeight: 32 }}>Send All</button>
    <button type="button" onClick={() => { setMissingComps(new Set()); setDamagedComps(new Set()); }} style={{ ...S.btnGhost, fontSize: 10, padding: '4px 10px', minHeight: 32 }}>Reset All</button>
  </div>}
  ...
  {form.status === 'unsorted' && missingComps.size === catComps.length && damagedComps.size === 0 && <p style={{ fontSize: 11, color: T.re, marginTop: 5, background: 'rgba(248,113,113,.06)', border: '1px solid rgba(248,113,113,.15)', borderRadius: 6, padding: '6px 10px' }}>All missing — change status to "Damaged" or deselect some.</p>}
      ...
      {exportPdfHtml && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: T.bg, display: 'flex', flexDirection: 'column', overscrollBehavior: 'contain' }}>
          ...
          <iframe ref={exportFrameRef} title="Inventory export preview" srcDoc={exportPdfHtml} style={{ flex: 1, border: 'none', width: '100%', background: T.bg }} />
          <div style={{ padding: '10px 16px', paddingBottom: 'max(10px, env(safe-area-inset-bottom))', background: 'rgba(8,11,20,.95)', borderTop: '1px solid rgba(255,255,255,.08)', display: 'flex', gap: 10, justifyContent: 'center' }}>
            ...
            <button onClick={() => printOrQueue('document', exportPdfHtml!, 'A4', 'Inventory Export', undefined, addToast, exportFrameRef.current)} style={{ ...S.btnPrimary, ...S.btnLg, flex: 1, maxWidth: 160 }}>Print / Share</button>
          </div>
        </div>,
        document.body
      )}
