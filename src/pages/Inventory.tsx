      ...
      <div style={{ background: 'rgba(255,255,255,0.015)', border: `1px solid ${T.bd}`, borderRadius: 8, overflow: 'hidden', display: loading && items.length === 0 ? 'none' : undefined }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 880 }}>
          ...
          <tbody>{paged.map((item) => {
            ...
            <td style={S.tdStyle}>{item.created_at && (() => { const d = Math.floor((Date.now() - new Date(item.created_at).getTime()) / 86400000); return <span style={{ fontSize: 10, fontWeight: 600, color: d > 30 ? T.re : d > 14 ? T.yl : T.tx3 }}>{d}d</span>; })()}</td>
              ...
              <div style={{ padding: '12px 14px', borderBottom: `1px solid ${T.bd}`, borderLeft: `3px solid ${STATUS_DOT_COLOR[item.status] || T.yl}`, display: 'flex', flexDirection: 'column', gap: 6, position: 'relative', WebkitUserSelect: 'none', userSelect: 'none' }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: T.tx3, marginTop: 2 }}>{item.size || '—'} · {item.location || '—'}{item.created_at && (() => { const d = Math.floor((Date.now() - new Date(item.created_at).getTime()) / 86400000); return d > 0 ? <span style={{ marginLeft: 6, padding: '1px 5px', borderRadius: 4, fontSize: 9, fontWeight: 600, background: d > 30 ? 'rgba(239,68,68,.1)' : d > 14 ? 'rgba(245,158,11,.1)' : 'rgba(255,255,255,.04)', color: d > 30 ? T.re : d > 14 ? T.yl : T.tx3 }}>{d}d</span> : null; })()}</div>
                  </div>
