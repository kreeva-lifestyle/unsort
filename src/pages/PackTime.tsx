    ...
    <div className="page-pad" style={{ fontFamily: T.sans, color: T.tx, minHeight: '100%', position: 'relative' }} onClick={focusInput}>
      ...
      {cameraOpen && (
        <div style={{ marginBottom: 12, borderRadius: 12, overflow: 'hidden', border: `1px solid ${T.ac33}`, position: 'relative', background: '#0a0e18' }}>
          ...
          <button type="button" onTouchEnd={(e) => { e.preventDefault(); stopCam(); setCameraOpen(false); }} onClick={() => { stopCam(); setCameraOpen(false); }} style={{ position: 'absolute', top: 8, right: 8, zIndex: 100, width: 36, height: 36, borderRadius: 8, background: 'rgba(0,0,0,.8)', border: '1px solid rgba(255,255,255,.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', fontSize: 18, fontWeight: 700, WebkitTapHighlightColor: 'transparent' }} aria-label="Close camera">✕</button>
        </div>
      )}
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <button onClick={() => { const v = awbInput.trim(); if (v) submitAwb(v); }} disabled={!awbInput.trim()} style={{ position: 'absolute', right: 5, top: '50%', transform: 'translateY(-50%)', width: 38, height: 38, borderRadius: 8, border: 'none', background: awbInput.trim() ? `linear-gradient(135deg, ${T.ac}, ${T.ac2})` : 'rgba(255,255,255,0.05)', color: '#fff', cursor: awbInput.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
