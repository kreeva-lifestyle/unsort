    ...
    <div style={{ animation: 'fi .3s ease both' }}>
        ...
        <div className="modal-inner" style={{ ...modal, width: 380 }} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div><label style={label}>How many to {adjustMode}?</label>
              <input type="number" min="1" value={adjustQty} onChange={e => setAdjustQty(e.target.value)} placeholder="0" style={input} /></div>
      ...
      {exportHtml && createPortal(
        <div style={{ position: 'fixed', inset: 0, zIndex: 10000, background: '#060810', display: 'flex', flexDirection: 'column', touchAction: 'none' }}>
          ...
          <iframe srcDoc={exportHtml} style={{ flex: 1, border: 'none', width: '100%', background: '#fff' }} />
          <div style={{ padding: '10px 16px', paddingBottom: 'max(10px, env(safe-area-inset-bottom))', background: 'rgba(8,11,20,.95)', borderTop: '1px solid rgba(255,255,255,.08)', display: 'flex', justifyContent: 'center' }}>
            <button onClick={() => setExportHtml(null)} style={{ padding: '10px 24px', borderRadius: 8, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(255,255,255,.04)', color: '#8896B0', fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>Close</button>
          </div>
        </div>,
        document.body
      )}
