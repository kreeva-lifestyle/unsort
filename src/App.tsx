...
const Inventory = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); const { error } = selected ? await supabase.from('inventory_items').update(form).eq('id', selected.id) : await supabase.from('inventory_items').insert({ ...form, reported_by: profile?.id }); if (error) addToast(error.message, 'error'); else { addToast(selected ? 'Updated!' : 'Added!', 'success'); setShowModal(false); setSelected(null); setForm({ product_id: '', serial_number: '', status: 'unsorted', location: '', notes: '' }); fetchData(); } };
  ...
  const openEdit = (item: any) => { setSelected(item); setForm({ product_id: item.product_id, serial_number: item.serial_number || '', status: item.status, location: item.location || '', notes: item.notes || '' }); setCatSearch(item.products?.name || ''); setShowModal(true); };
  ...
  return (
    <div style={{ padding: '22px 26px', animation: 'fi .18s ease' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        {canEdit && <div onClick={() => { setSelected(null); setForm({ product_id: '', serial_number: '', status: 'unsorted', location: '', notes: '' }); setCatSearch(''); setShowModal(true); }} style={S.btnPrimary}>+ Add Item</div>}
      </div>
