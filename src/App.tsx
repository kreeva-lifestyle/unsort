  ...
  return (
    <div style={{ width: 230, height: '100vh', background: T.s, borderRight: `1px solid ${T.bd}`, display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, zIndex: 100, overflowY: 'auto' }}>
      ...
      <div style={{ padding: '14px 16px', borderTop: `1px solid ${T.bd}`, marginTop: 'auto' }}>
        ...
        <p style={{ margin: '10px 0 0', fontSize: 9, color: T.tx3, letterSpacing: 1, textTransform: 'uppercase' as const, textAlign: 'center', opacity: 0.6 }}>Powered by Arya Designs</p>
      </div>
    </div>
  );
};
...
const Inventory = () => {
  ...
  return (
    <div style={{ padding: '22px 26px', animation: 'fi .18s ease' }}>
      ...
      {showModal && (<div style={modalOverlay}><div style={modalBox}><div style={modalHead}><span style={{ fontSize: 15, fontWeight: 600, color: T.tx }}>{selected ? 'Edit' : 'Add'} Item</span><span onClick={() => setShowModal(false)} style={{ cursor: 'pointer', color: T.tx3, fontSize: 20, lineHeight: 1 }}>✕</span></div><form onSubmit={handleSubmit} style={{ padding: 21 }}><div style={{ marginBottom: 14 }}><label style={fLabel}>Product *</label><select value={form.product_id} onChange={(e) => setForm({ ...form, product_id: e.target.value })} required style={fInput}><option value="">Select</option>{products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 13, marginBottom: 14 }}><div><label style={fLabel}>Serial Number</label><input value={form.serial_number} onChange={(e) => setForm({ ...form, serial_number: e.target.value })} placeholder="Optional" style={fInput} /></div><div><label style={fLabel}>Batch Number</label><input value={form.batch_number} onChange={(e) => setForm({ ...form, batch_number: e.target.value })} placeholder="Optional" style={fInput} /></div></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 13, marginBottom: 14 }}><div><label style={fLabel}>Status</label><select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} style={fInput}><option value="unsorted">Unsorted</option><option value="damaged">Damaged</option><option value="complete">Complete</option></select></div><div><label style={fLabel}>Location</label><input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="e.g. Warehouse A" style={fInput} /></div></div><div style={{ marginBottom: 14 }}><label style={fLabel}>Notes</label><input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Optional notes" style={fInput} /></div><div style={{ padding: '15px 0 0', borderTop: `1px solid ${T.bd}`, display: 'flex', justifyContent: 'flex-end', gap: 9 }}><button type="button" onClick={() => setShowModal(false)} style={btnGhost}>Cancel</button><button type="submit" style={btnPrimary}>{selected ? 'Update' : 'Add'}</button></div></form></div></div>)}
...
const Products = () => {
  ...
  return (
    <div style={{ padding: '22px 26px', animation: 'fi .18s ease' }}>
      ...
      {showModal && (<div style={modalOverlay}><div style={modalBox}><div style={modalHead}><span style={{ fontSize: 15, fontWeight: 600, color: T.tx }}>{selected ? 'Edit' : 'Add'} Product</span><span onClick={() => setShowModal(false)} style={{ cursor: 'pointer', color: T.tx3, fontSize: 20, lineHeight: 1 }}>✕</span></div><form onSubmit={handleSubmit} style={{ padding: 21 }}><div style={{ marginBottom: 14 }}><label style={fLabel}>SKU *</label><input value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })} required style={fInput} /></div><div style={{ marginBottom: 14 }}><label style={fLabel}>Name *</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required style={fInput} /></div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 13, marginBottom: 14 }}><div><label style={fLabel}>Category</label><input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="e.g. Lehenga, Saree" style={fInput} /></div><div><label style={fLabel}>Description</label><input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Optional" style={fInput} /></div></div><div style={{ padding: '15px 0 0', borderTop: `1px solid ${T.bd}`, display: 'flex', justifyContent: 'flex-end', gap: 9 }}><button type="button" onClick={() => setShowModal(false)} style={btnGhost}>Cancel</button><button type="submit" style={btnPrimary}>{selected ? 'Update' : 'Add'}</button></div></form></div></div>)}
