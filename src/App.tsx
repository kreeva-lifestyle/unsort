    ...
    <div className="main-area" style={{ marginLeft: sidebarOpen ? 220 : 0, display: 'flex', flexDirection: 'column', maxWidth: '100vw', transition: 'margin-left .25s cubic-bezier(.4,0,.2,1)' }}>
      ...
      <div className="mobile-hamburger" style={{ display: 'none', position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 102, background: T.s, borderTop: `1px solid ${T.bd}`, padding: '8px 0', justifyContent: 'space-around' }}>
        {[{ id: 'dashboard', icon: 'grid', label: 'Home' }, { id: 'inventory', icon: 'box', label: 'Inventory' }, { id: 'packtime', icon: 'scan', label: 'PackStation' }, { id: 'challan', icon: 'file', label: 'Challan' }].filter(t => checkTab(t.id)).map(t => (
          <a key={t.id} href={`#/${t.id}`} aria-current={tab === t.id ? 'page' : undefined} onClick={e => { if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey || e.button !== 0) return; e.preventDefault(); setTab(t.id); setMobileMenu(false); }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', textDecoration: 'none', padding: '2px 16px', color: tab === t.id ? T.ac : T.tx3, fontSize: 10, fontWeight: 500, transition: 'color .2s ease', position: 'relative', WebkitTouchCallout: 'none', WebkitUserSelect: 'none', userSelect: 'none' }}>
        ...
        <button type="button" onClick={() => setMobileMenu(true)} aria-haspopup="dialog" aria-expanded={mobileMenu} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'pointer', padding: '2px 16px', background: 'none', border: 0, font: 'inherit', color: (mobileMenu || !['dashboard', 'inventory', 'packtime', 'challan'].includes(tab)) ? T.ac : T.tx3, fontSize: 10, fontWeight: 500 }}>
