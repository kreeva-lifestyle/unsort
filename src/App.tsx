...
// Shared styles
const S = {
  fLabel: { fontSize: 11, color: T.tx3, marginBottom: 6, textTransform: 'uppercase' as const, letterSpacing: 1.2, display: 'block', fontWeight: 500 } as React.CSSProperties,
  fInput: { width: '100%', background: T.s2, border: `1px solid ${T.bd2}`, borderRadius: T.r, color: T.tx, fontFamily: T.sans, fontSize: 13, padding: '10px 14px', outline: 'none', transition: 'border-color .2s, box-shadow .2s' } as React.CSSProperties,
  btnPrimary: { padding: '8px 20px', borderRadius: T.r, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: T.sans, background: `linear-gradient(135deg, ${T.ac}, ${T.ac2})`, color: '#fff', display: 'inline-flex', alignItems: 'center', gap: 6, letterSpacing: 0.3, boxShadow: '0 2px 8px rgba(139,92,246,.25)' } as React.CSSProperties,
  btnGhost: { padding: '8px 18px', borderRadius: T.r, border: `1px solid ${T.bd2}`, cursor: 'pointer', fontSize: 13, fontWeight: 500, fontFamily: T.sans, background: 'transparent', color: T.tx2, display: 'inline-flex', alignItems: 'center', gap: 6 } as React.CSSProperties,
  btnDanger: { padding: '6px 14px', borderRadius: T.r, border: '1px solid rgba(248,113,113,.25)', cursor: 'pointer', fontSize: 12, fontWeight: 500, fontFamily: T.sans, background: 'rgba(248,113,113,.08)', color: T.re } as React.CSSProperties,
  modalOverlay: { position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, backdropFilter: 'blur(8px)' } as React.CSSProperties,
  modalBox: { background: T.s, border: `1px solid ${T.bd2}`, borderRadius: 16, width: 520, maxWidth: 'calc(100vw - 32px)', maxHeight: '88vh', overflowY: 'auto' as const, boxShadow: '0 24px 64px rgba(0,0,0,.5)' } as React.CSSProperties,
  modalHead: { padding: '18px 22px', borderBottom: `1px solid ${T.bd}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' } as React.CSSProperties,
  thStyle: { fontSize: 10, color: T.tx3, textTransform: 'uppercase' as const, letterSpacing: 1.2, padding: '11px 14px', textAlign: 'left' as const, fontWeight: 600, borderBottom: `1px solid ${T.bd}`, background: T.s2 } as React.CSSProperties,
  tdStyle: { padding: '11px 14px', fontSize: 13, borderBottom: `1px solid ${T.bd}`, color: T.tx } as React.CSSProperties,
};
...
const Sidebar = ({ activeTab, setActiveTab }: { activeTab: string; setActiveTab: (t: string) => void }) => {
  ...
  return (
    <div style={{ width: 240, height: '100vh', background: `linear-gradient(180deg, ${T.s} 0%, ${T.bg} 100%)`, borderRight: `1px solid ${T.bd}`, display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, zIndex: 100, overflowY: 'auto' }}>
      <div style={{ padding: '22px 20px 18px', borderBottom: `1px solid ${T.bd}` }}>
        <div style={{ fontSize: 20, fontWeight: 700, fontFamily: T.mono, letterSpacing: -0.5, background: `linear-gradient(135deg, ${T.ac}, ${T.ac2})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Unsort</div>
        <div style={{ fontSize: 10, color: T.tx3, letterSpacing: 2.5, textTransform: 'uppercase' as const, marginTop: 4 }}>Product Tracking</div>
      </div>
      <div style={{ fontSize: 9, color: T.tx3, letterSpacing: 2.5, textTransform: 'uppercase' as const, padding: '18px 20px 8px', fontWeight: 600 }}>Menu</div>
      <nav style={{ flex: 1, padding: '4px 10px 10px' }}>
        {tabs.map((t) => (
          <div key={t.id} onClick={() => setActiveTab(t.id)} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 14px', margin: '2px 0', cursor: 'pointer', background: activeTab === t.id ? 'rgba(139,92,246,.1)' : 'transparent', color: activeTab === t.id ? T.ac : T.tx2, fontSize: 13, fontWeight: activeTab === t.id ? 600 : 400, fontFamily: T.sans, borderRadius: T.r, transition: 'all .15s', boxShadow: activeTab === t.id ? '0 0 12px rgba(139,92,246,.08)' : 'none' }}>
            <span style={{ width: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: activeTab === t.id ? 1 : 0.6 }}><Icon name={t.icon} size={17} /></span> {t.label}
          </div>
        ))}
      </nav>
      <div style={{ padding: '16px 14px', borderTop: `1px solid ${T.bd}`, marginTop: 'auto' }}>
        {profile && <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, padding: '10px 12px', background: T.s2, borderRadius: T.r }}>
          <div style={{ width: 34, height: 34, borderRadius: 10, background: `linear-gradient(135deg, ${T.ac}, ${T.ac2})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#fff', flexShrink: 0 }}>{(profile.full_name || 'U')[0].toUpperCase()}</div>
          <div style={{ flex: 1, minWidth: 0 }}><p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: T.tx, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{profile.full_name}</p><p style={{ margin: 0, fontSize: 10, color: T.tx3, textTransform: 'capitalize' as const }}>{profile.role}</p></div>
        </div>}
        <div onClick={handleSignOut} style={{ width: '100%', padding: '9px 14px', borderRadius: T.r, background: 'rgba(248,113,113,.06)', border: '1px solid rgba(248,113,113,.15)', color: T.re, cursor: 'pointer', fontSize: 12, fontWeight: 500, fontFamily: T.sans, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>Sign Out</div>
        <p style={{ margin: '14px 0 0', fontSize: 8, color: T.tx3, letterSpacing: 2, textTransform: 'uppercase' as const, textAlign: 'center', opacity: 0.4 }}>Powered by Arya Designs</p>
      </div>
    </div>
  );
};
...
const Inventory = ({ globalSearch = '' }: { globalSearch?: string }) => {
  ...
  const [form, setForm] = useState({ product_id: '', serial_number: '', status: 'unsorted', location: '', notes: '', order_id: '', marketplace: '', ticket_id: '', link: '' });
  ...
  const fetchData = () => {
    ...
    supabase.from('item_components').select('inventory_item_id, status, components(name)').eq('status', 'missing').then(({ data }) => {
      const map: Record<string, string[]> = {};
      (data || []).forEach((ic: any) => { if (!map[ic.inventory_item_id]) map[ic.inventory_item_id] = []; if (ic.components?.name) map[ic.inventory_item_id].push(ic.components.name); });
      setItemMissing(map);
    });
  };
  ...
  const openEdit = async (item: any) => {
    setSelected(item); setForm({ product_id: item.product_id, serial_number: item.serial_number || '', status: item.status, location: item.location || '', notes: item.notes || '', order_id: item.order_id || '', marketplace: item.marketplace || '', ticket_id: item.ticket_id || '', link: item.link || '' }); setCatSearch(item.products?.name || '');
  ...
  const printBarcode = (uniqueId: string, categoryName: string, skuCode: string) => {
    ...
    if (!win) return;
    win.document.write(`<html><head><title>Barcode - ${uniqueId}</title><style>body{font-family:'IBM Plex Sans',sans-serif;text-align:center;padding:20px}@media print{.no-print{display:none}}</style></head><body><p style="font-size:14px;font-weight:600;margin:0 0 4px">${categoryName}</p>${skuCode ? `<p style="font-size:11px;color:#666;margin:0 0 12px">${skuCode}</p>` : ''}<img src="${canvas.toDataURL()}" /><br><button class="no-print" onclick="window.print()" style="margin-top:16px;padding:8px 24px;font-size:14px;cursor:pointer">Print</button></body></html>`);
    win.document.close();
  };
  ...
  const filtered = items.filter((i) => {
    ...
    if (searchTerm) {
      ...
      const fields = [
        i.products?.name, i.products?.sku, i.batch_number, i.serial_number,
  ...
  return (
    <div style={{ padding: '22px 26px', animation: 'fi .18s ease' }}>
      ...
      <div style={{ background: T.s, border: `1px solid ${T.bd}`, borderRadius: 12, padding: '12px 16px', marginBottom: 14, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10 }}>
          ...
          <tbody>{filtered.map((item) => {
            ...
            <td style={S.tdStyle}>
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                <span onClick={() => openComps(item)} style={{ ...S.btnPrimary, padding: '4px 10px', fontSize: 11 }}>View</span>
                {item.batch_number && <span onClick={() => printBarcode(item.batch_number, item.products?.name || '', item.serial_number || '')} style={{ ...S.btnGhost, padding: '4px 10px', fontSize: 11 }}>Barcode</span>}
