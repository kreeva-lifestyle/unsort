...
const Inventory = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setShowModal(false); setSelected(null); setForm({ product_id: '', serial_number: '', status: 'unsorted', location: '', notes: '' }); setCatComps([]); setMissingComps(new Set()); setTagInput(''); fetchData();
  ...
  return (
    <div style={{ padding: '22px 26px', animation: 'fi .18s ease' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        {canEdit && <div onClick={() => { setSelected(null); setForm({ product_id: '', serial_number: '', status: 'unsorted', location: '', notes: '' }); setCatSearch(''); setCatComps([]); setMissingComps(new Set()); setTagInput(''); setShowModal(true); }} style={S.btnPrimary}>+ Add Item</div>}
      </div>
          ...
          <tbody>{filtered.map((item) => {
            ...
            <td style={S.tdStyle}>
              <div style={{ display: 'flex', gap: 6 }}>
                <span onClick={() => openComps(item)} style={{ ...S.btnPrimary, padding: '4px 12px', fontSize: 11 }}>View Parts</span>
